		...
		for(int i = 0; i < sensorData.bodyCount; i++)
		{
			   ...
			   (maxUserDistance <= 0f || Mathf.Abs(bodyData.position.z) <= maxUserDistance))
			{
				   ...
				   bodyData.joint[(int)KinectInterop.JointType.ThumbLeft].trackingState != KinectInterop.TrackingState.NotTracked)
				{
					...
					bodyData.leftThumbForward = Vector3.forward;
				   ...
				   bodyData.joint[(int)KinectInterop.JointType.ThumbRight].trackingState != KinectInterop.TrackingState.NotTracked)
				{
					...
					bodyData.rightThumbForward = Vector3.forward;
			...
			if(bJointValid)
			{
				...
				if(nextJoint != joint && nextJoint >= 0 && nextJoint < sensorData.jointCount)
				{
					...
					if(bNextJointValid)
					{
						...
						else if((joint == (int)KinectInterop.JointType.WristLeft) || (joint == (int)KinectInterop.JointType.HandLeft))
						{
						...
						else if((joint == (int)KinectInterop.JointType.WristRight) || (joint == (int)KinectInterop.JointType.HandRight))
						{
							Quaternion armTurnRotation = Quaternion.identity;
